...
describe('validatePhone function', () => {
    ...
    it('should return false and provide correct error message for invalid phone numbers', () => {
        ...
        expect(result1.errorMessage).toBe("Phone Error: Phone number must start with 0 and be 10 or 11 digits long");

        const result2 = validatePhone("1234");
        expect(result2.isValid).toBe(false);
        expect(result2.errorMessage).toBe("Phone Error: Phone number must start with 0 and be 10 or 11 digits long");

        const result3 = validatePhone("abcdefghij");
        expect(result3.isValid).toBe(false);
        expect(result3.errorMessage).toBe("Phone Error: Phone number must start with 0 and be 10 or 11 digits long");
    });
});
